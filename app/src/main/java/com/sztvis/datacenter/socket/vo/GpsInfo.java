...
public class GpsInfo extends DataBase {
    ...
    public void parseBody(byte[] message) {
        ...
        if (nmea.indexOf("$GNRMC") > -1 || nmea.indexOf("$GNGGA") > -1) {
            ...
            BaseApplication.gpsList.add(nmea);
        }
        this.parseGPRMC();
    ...
    public int parseGPRMC() {
        if (BaseApplication.gpsList.size() == 2) {
            for (int i = 0; i < BaseApplication.gpsList.size(); i++) {
                String[] bodys = BaseApplication.gpsList.get(i).split(",");
                if (bodys[0].equals("$GNRMC") && bodys[2].equals("A")) {
                    ...
                    double g = getLongitude() / 15;
                    long timeZone = Math.round(g);
                    long h = bodys[6].equals("E") ? timeZone : -timeZone;
                    String t2 = DateUtil.addHour(this.getTime(), (int) h);
                    this.setTime(t2);
                }
            ...
            BaseApplication.currentGps = this;
            BaseApplication.gpsList.clear();
            return 0;
        }
        return 1;
    }
